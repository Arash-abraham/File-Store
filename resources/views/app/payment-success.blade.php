...
<body class="flex items-center justify-center p-4">
        ...
        <div class="success-animation">
            <div class="order-card p-6 md:p-8 text-center"> 
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 mb-6 border border-blue-200">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-right">
                        ...
                        <div class="bg-white rounded-lg p-3 shadow-sm">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="font-bold text-green-600">{{ number_format($order->remaining_amount) }} تومان</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-lg p-3 shadow-sm">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="font-bold text-gray-800">{{ verta($order->created_at)->format('Y/m/d') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 bg-white rounded-lg p-3 shadow-sm">
                        <div class="space-y-1">                            
                            ...
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600 text-sm">مبلغ کل: {{ number_format($order->total_amount) }} تومان</span>
                            </div>

                            @if($order->discount_amount > 0)
                                <div class="flex justify-between items-center">
                                    <span class="text-red-600 text-sm">تخفیف: {{ number_format($order->discount_amount) }} تومان</span>
                                </div>
                            @endif

                            @if($order->paid_from_wallet > 0)
                                <div class="flex justify-between items-center">
                                    <span class="text-blue-600 text-sm">پرداخت از کیف پول: {{ number_format($order->paid_from_wallet) }} تومان</span>
                                </div>
                            @endif
                    ...
                    <div class="mt-3 flex justify-center">
                        <div class="discount-badge px-3 py-1 rounded-full font-semibold text-xs shadow-lg">
                            ...
                            {{ number_format(($order->discount_amount / $order->total_amount) * 100, 1) }}% تخفیف
                        </div>
                    </div>
                    @endif
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-center">
                    <a href="{{ route('home') }}" class="btn-primary text-white px-6 py-3 rounded-lg font-semibold text-base inline-flex items-center justify-center">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const hasDiscount = {{ $order->discount_amount > 0 ? 'true' : 'false' }};
