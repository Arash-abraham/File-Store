        ...
        <div class="space-y-6">
                ...
                <div class="bg-gradient-to-r from-white to-green-50 border border-green-100 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                        <div class="flex items-center space-x-4 space-x-reverse mb-4 lg:mb-0">
                            ...
                            <div>
                                <h3 class="font-bold text-gray-800 text-lg">خرید #{{ $payment->id }}</h3>
                        ...
                        <div class="flex items-center space-x-4 space-x-reverse">
                            ...
                            <div class="text-right">
                                <p class="text-lg font-bold text-gray-800">
                                    {{ number_format($payment->amount ?? $payment->order->total_amount ?? 0) }} تومان
                                </p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
                            @forelse($payment->order->items ?? [] as $item)
                                <div class="bg-white rounded-xl p-4 border border-gray-200 hover:border-green-300 transition-all duration-300 shadow-sm hover:shadow-md">
                                    <a href="{{ route('show-product', $item->product->id) }}">
                                        <div class="mb-3">
                                            @if($item->product && $item->product->image_urls && count($item->product->image_urls) > 0)
                                                <img src="{{ asset($item->product->image_urls[0]) }}" alt="{{ $item->product->title }}" 
            ...
            @empty
                ...
                <div class="text-center py-16">
                    ...
                    <a href="{{ route('products') }}" class="bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 transform hover:scale-105 shadow-lg inline-flex items-center">
