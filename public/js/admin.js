...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupCategoryModal() {
        ...
        slugInput.addEventListener('input', updateCategoryPreview);
    ...
    function resetCategoryForm() {
        ...
        firstIcon.classList.add('border-blue-500', 'bg-blue-50');
        firstIcon.classList.remove('border-gray-200');
        firstIcon.querySelector('i').classList.add('text-blue-600');
        ...
        // Reset hidden inputs
        document.getElementById('selectedIcon').value = 'fas fa-laptop-code';
    ...
    function updateCategoryPreview() {
        const name = document.querySelector('input[name="name"]').value || 'نام دسته‌بندی';
        const description = document.querySelector('textarea[name="description"]').value || 'توضیحات دسته‌بندی...';
        const icon = document.getElementById('selectedIcon').value;
        const color = document.getElementById('selectedColor').value;
        
        document.getElementById('previewName').textContent = name;
        document.getElementById('previewDescription').textContent = description;
        document.getElementById('previewIcon').className = `${icon} text-${color}-600 text-2xl ml-3`;
    }
    ...
    function handleCategorySubmission(e) {
        ...
        if (sampleCategories.find(c => c.slug === slug)) {
        ...
        // Show success message
        showNotification('دسته‌بندی جدید با موفقیت اضافه شد', 'success');
    ...
    window.editCategory = function(categoryId) {
        ...
        document.getElementById('editPreviewCount').textContent = `${category.productCount} محصول`;
        ...
        document.querySelectorAll('.edit-icon-option').forEach(opt => {
            ...
            if (opt.dataset.icon === category.icon) {
                ...
                opt.querySelector('i').classList.remove('text-gray-600');
            }
        });
        ...
        document.querySelectorAll('.edit-color-option').forEach(opt => {
            ...
            if (opt.dataset.color === category.color) {
                ...
                opt.classList.add(`border-${category.color}-200`);
            }
        });
    ...
    window.deleteCategory = function(categoryId) {
        if (confirm('آیا از حذف این دسته‌بندی مطمئن هستید؟')) {
            ...
            if (categoryIndex !== -1) {
                ...
                const productCount = sampleProducts.filter(p => p.category === category.slug).length;
                if (productCount > 0) {
                    showNotification(`نمی‌توان دسته‌بندی را حذف کرد. ${productCount} محصول در این دسته وجود دارد`, 'error');
                    return;
                }
